...
function AddMainCategory() {
  ...
  const fetchData = () => {
    const profileUrl = `${config.app.api_url}/profil`;

    const getProfiles = axios.get(profileUrl, {
      headers: { Authorization: "Bearer " + user.token },
    });
    axios.all([getProfiles]).then(
      axios.spread(async (...allData) => {
        const allProfiles = allData[0].data;
        let profil = [];
        await allProfiles.data.forEach((item) => {
          profil.push({ label: item.name, value: item._id });
        });
        setProfiles(profil);
      })
    );
  ...
  const [loading, postData] = usePostData(`main-category`, "POST");
  const addMainCategory = async (e) => {
    e.preventDefault();

    const data = new FormData();
    data.append("name", name);
    data.append("profils", JSON.stringify(profile.map((i) => i["value"])));
